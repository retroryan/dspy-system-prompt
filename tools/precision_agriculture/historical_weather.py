    ...
    def validate_date_range(cls, v, info):
        if 'start_date' in info.data:
            ...
            if end_date > min_historical_date:
                raise ValueError(f"Historical data only available for dates at least 5 days in the past. Latest available date: {min_historical_date.strftime('%Y-%m-%d')}")
...
Daily Historical Summary:"""
            ...
            if daily and "time" in daily:
                for i in range(len(daily.get("time", []))):
                    ...
                    uv_max = daily.get("uv_index_max", [])[i] if i < len(daily.get("uv_index_max", [])) else "N/A"
            ...
            if daily:
                temp_maxes = [t for t in daily.get("temperature_2m_max", []) if t is not None]
