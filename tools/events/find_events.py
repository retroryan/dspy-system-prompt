...
class EventSearchRequest(BaseModel):
    location: Optional[str] = Field(
        default=None,
        description="City or venue where events are taking place"
    )
    ...
    event_type: Optional[str] = Field(
        default=None,
        description="Type of event (concert, sports, conference, etc.)"
    )
...
class FindEventsTool(BaseTool):
    ...
    def _filter_events_by_date(self, events: list, date_filter: str) -> list:
        ...
        for event in events:
            ...
            event_date_to = datetime.fromisoformat(event['dateTo']) if event.get('dateTo') else event_date_from
            ...
            if date_filter_lower in ['march', 'mar']:
                ...
                if event_date_from.year == 2025 or event_date_to.year == 2025:
                    filtered_events.append(event)
            elif date_filter in event['dateFrom'] or (event.get('dateTo') and date_filter in event['dateTo']):
                filtered_events.append(event)
            else:
        ...
        try:
            ...
            if location:
                ...
                if normalized_location in events_data:
                    ...
                    if date:
                        location_events = self._filter_events_by_date(location_events, date)
                    ...
                    for event in location_events:
                        ...
                        if event_type:
                            ...
                            if event_type.lower() in event_desc_lower or event_type.lower() in event_name_lower:
                ...
                for city, city_events in events_data.items():
                    ...
                    # Filter by date if specified
                    if date:
                        city_events_filtered = self._filter_events_by_date(city_events_filtered, date)
            ...
            return {
                ...
                "available_locations": list(events_data.keys()) if not location else None
            }
            
        except Exception as e:
            return {
                "error": f"Error searching for events: {str(e)}"
            }
